...
const removeItem = function (item) {
  const item_time = item.firstElementChild.getAttribute('data-time')
  const removeIndex = todoItems.findIndex((dt) => String(dt.addedAt) === String(item_time));
...
const handleItem = function (itemData) {
  ...
  items.forEach((item) => {
    if (
      ...
      //open
      if (item.querySelector("[data-open]")) {
        item.querySelector("[data-open]").addEventListener("click", function (e) {
          e.preventDefault();
          const itemIndex = todoItems.indexOf(itemData);
          const currentItem = todoItems[itemIndex];
          const currentClass = currentItem.isOpen
            ? "bi-arrow-bar-up"
            : "bi-arrow-bar-down";
          currentItem.isOpen = currentItem.isOpen ? false : true;
          todoItems.splice(itemIndex, 1, currentItem);
          // todoItems.splice(itemIndex, noofelem, element);
          setLocalStorage(todoItems);
          //console.log(todoItems[itemIndex]);
          const iconClass = currentItem.isOpen
            ? "bi-arrow-bar-up"
            : "bi-arrow-bar-down";
...
const getList = function (todoItems) {
  ...
  if (todoItems.length > 0) {
    todoItems.forEach((item) => {
      const is_bigger = item.name.length > 60
      const is_open = item.isOpen
      ...
      itemList.insertAdjacentHTML(
        ...
        `<li class="list-group-item d-flex justify-content-between align-items-center">
          ${!item.isOpen ? `<span class= "title" data-time="${item.addedAt}" > ${is_bigger ? item.name.slice(0, 60) + '...' : item.name}</span >` : ''}
          ${is_bigger && item.isOpen ? `<span class="title" data-time="${item.addedAt}">${item.name}</span>` : ''}
          <span>
              ${is_bigger ? `<a href="#" data-open><i class="bi ${item.isOpen ? 'bi-arrow-bar-up' : 'bi-arrow-bar-down'} blue"></i></a>` : ''}
